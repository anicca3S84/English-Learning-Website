        ...
        <div class="container mt-5 mb-5">
            ...
            <div class="custom-flex-row">
                <div class="left-col">
                    ...
                    <div style="margin-top: 40px; display: flex; align-items: center;" class="button-lesson">
                        <form action="{{ route('lesson.finish', ['id' => $lesson->id]) }}" method="POST">
                            @csrf
                            <button type="submit" class="btn">Finish</button>
                        </form>

                        <div style="margin-left: 20px;">
                            <a href="{{ route('course.show', ['skillSlug' => $skillSlug, 'courseSlug' => $courseSlug]) }}"
                                class="btn">
                    ...
                    <div style="margin-top: 48px;">
                        <h2 style="color: #23085A; font-size: 2rem; font-weight: bold; margin-bottom: 16px;">Language
                            level</h2>
                        <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 8px;">
                            <span
                                style="background-color: #008738; color: white; padding: 8px 18px; border-radius: 4px; font-size: 1.1rem; font-weight: 500;">A1
                                Elementary</span>
                        </div>
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 8px;">
                            <!-- Interactive Stars -->
                            <span id="star-1" data-star="1" class="rating-star"
                                style="cursor:pointer; color: #bbb; font-size: 1.5rem;">★</span>
                            <span id="star-2" data-star="2" class="rating-star"
                                style="cursor:pointer; color: #bbb; font-size: 1.5rem;">★</span>
                            <span id="star-3" data-star="3" class="rating-star"
                                style="cursor:pointer; color: #bbb; font-size: 1.5rem;">★</span>
                            <span id="star-4" data-star="4" class="rating-star"
                                style="cursor:pointer; color: #bbb; font-size: 1.5rem;">★</span>
                            <span id="star-5" data-star="5" class="rating-star"
                                style="cursor:pointer; color: #bbb; font-size: 1.5rem;">★</span>
                            <span style="margin-left: 8px; color: #444; font-size: 0.95rem;">Average: 4.2 (138
                                votes)</span>
                        </div>
                        ...
                        <div style="margin-top: 16px;">
                            ...
                            @if ($lesson->comments->count() > 7)
                                <div id="load-more-wrapper"
                                    style="width:100%; text-align:center; background:#d2cfdf; padding: 20px 0 22px 0; border-radius: 0 0 28px 28px; margin-bottom: 26px;">
                                    <button id="load-more-btn"
                                        style="background:#2b0060; color:#fff; border:none; outline:none; padding:10px 34px; font-size:1.11rem; font-weight:500; border-radius: 22px; cursor:pointer;">Load
                            ...
                            <form id="comment-form">
                                @csrf
                                <div
                                    style="background:#f5f7fa; padding:40px 16px 32px 16px; border-radius: 0 0 32px 32px; margin-bottom:40px;">
                                    <h2 style="font-size:2rem; font-weight:900; color:#2b0060; margin-bottom:18px;">Add
                                        new comment</h2>

                                    <label for="ckeditor-comment"
                                        style="display:block; font-size:1.1rem; font-weight:700; color:#211658; margin-bottom:9px;">
                                    ...
                                    @auth
                                        <button type="submit"
                                            style="margin-top:24px; background:#2b0060; color:#fff; border:none; outline:none; padding:13px 48px; border-radius: 22px; font-size:1.13rem; font-weight:600; cursor:pointer;">
                                            Save
                                        </button>
                                    @endauth
                                    @guest
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        CKEDITOR.instances['ckeditor-comment'].setData(
                            ''); // Clear CKEditor sau khi gửi
                        $('#comment-error').text('');
                    }
                },
                error: function(xhr) {
